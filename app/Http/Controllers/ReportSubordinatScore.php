...
class ReportSubordinatScore extends Controller
{
    public function __construct(){
        $this->middleware(function ($request, $next) {
            if(!AccessControlService::checkAccessControl('Report3')){
                abort(403);
            }
            return $next($request);
        });
    ...
    public function getData(Request $request)
    {
        ...
        $client   = new Client();
        $responseReport = $client->post(env('URL_API').'get-report-subordinatscore',['form_params' => $input,'verify' => false,]);
